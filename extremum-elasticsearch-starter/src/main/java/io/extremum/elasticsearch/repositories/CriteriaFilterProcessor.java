...
class CriteriaFilterProcessor {
    ...
    private QueryBuilder processCriteriaEntry(Criteria.OperationKey key, Object value, String fieldName) {
        ...
        switch (key) {
            case WITHIN: {
                ...
                Assert.isTrue(
                        valArray[0] instanceof GeoPoint || valArray[0] instanceof String || valArray[0] instanceof Point,
                ...
                if (valArray[0] instanceof GeoPoint) {
                    ...
                    geoDistanceQueryBuilder.point(loc.getLat(), loc.getLon()).distance(dist.toString()).geoDistance(
                            GeoDistance.PLANE);
                    ...
                    if (loc.contains(",")) {
                        String c[] = loc.split(",");
                        geoDistanceQueryBuilder.point(Double.parseDouble(c[0]), Double.parseDouble(c[1])).distance(
                                dist.toString()).geoDistance(GeoDistance.PLANE);
                    } else {
    ...
    private void oneParameterBBox(GeoBoundingBoxQueryBuilder filter, Object value) {
        ...
        filter.setCorners(geoBBox.getTopLeft().getLat(), geoBBox.getTopLeft().getLon(),
                geoBBox.getBottomRight().getLat(), geoBBox.getBottomRight().getLon());
    }
    ...
    private List<QueryBuilder> buildNegationFilter(String fieldName, Iterator<Criteria.CriteriaEntry> it) {
        ...
        while (it.hasNext()) {
            ...
            QueryBuilder notFilter = QueryBuilders.boolQuery().mustNot(
                    processCriteriaEntry(criteriaEntry.getKey(), criteriaEntry.getValue(), fieldName));
