...
class ReferenceAndOwnedCollectionsReachableFromIt implements ReferenceCollector {
    ...
    private void collectReferencesFromTopTo(List<ReferenceContext> collectedRefs) {
        for (Object topElement : reference.getTop()) {
            if (topElement instanceof ResponseDto) {
                ...
                collectedRefs.addAll(new OwnedCollectionReferenceCollector(responseDto).collectReferences());
            }
        }
    }
}
